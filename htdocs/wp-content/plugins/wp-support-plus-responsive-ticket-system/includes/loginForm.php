...
<?php if($generalSettings['default_login_module']==1){?>
    <div id="wpspLoginAndSignUpDiv">
            ...
            <?php }?>
            <?php
            if(get_option( 'wpsp_ticket_open_page_shortcode' )){
                $open_ticket_page=get_permalink(get_option( 'wpsp_ticket_open_page_shortcode' ));
                echo __("Have ticket ID?",'wp-support-plus-responsive-ticket-system').' '
                        .'<a href="'.$open_ticket_page.'">'.__('Click Here','wp-support-plus-responsive-ticket-system').'</a> '
                        .__('to get ticket link','wp-support-plus-responsive-ticket-system');
            }
            ?>
    </div>
