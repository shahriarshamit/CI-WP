	  	...
	  	<tr>
			...
			<td style="display:none;"><?php echo $status->id;?></td>
	  		<td>
				...
				if(!in_array($status->name,$status_names)){
				?>
				<img alt="Delete" onclick="delete_custom_status(<?php echo $status->id;?>);" style="cursor: pointer;" title="<?php _e('Delete','wp-support-plus-responsive-ticket-system');?>" src="<?php echo WCE_PLUGIN_URL;?>asset/images/delete.png" >
				<?php 
				}
				?>
			</td>
			...
			?>
	  		...
	  		<td><img alt="save_color" onclick="save_custom_status_color(<?php echo $status->id;?>);" style="cursor: pointer;" title="<?php _e('Update','wp-support-plus-responsive-ticket-system');?>" src="<?php echo WCE_PLUGIN_URL;?>asset/images/save.png" > <span class="custom-status-color-saved" id="custom-status-color-saved-<?php echo $status->id;?>">Saved!</span></td>
	  		<?php
	  		/* END CLOUGH I.T. SOLUTIONS MODIFICATION
			*/
			?>
